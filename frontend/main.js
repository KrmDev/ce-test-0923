...
yellowStar.addEventListener('click', function() {
  animateStar(yellowStar, true);
  sendStarData("remove", 1, 42); 
});
...
function sendStarData(endpoint, mentorId, studentId) {
  const url = `http://localhost/star/${endpoint}`;
  const data = JSON.stringify({ idMentor: mentorId, idStudent: studentId });
